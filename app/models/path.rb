class Path
  ...
  validates_presence_of :value, message: "must be valid JSON data"
  ...
  def save
    if valid?
      resp = $doozer.set self.name, value.to_json, self.class.current_rev
  ...
  def self.find(path)
    ...
    if resp.err_code == Fraggle::Block::Response::Err::ISDIR
      ...
      new(name: path, type: 'file', value: resp.value, rev: resp.rev)
    end
  end
  ...
  def attribute=(k,v)
    v = (ActiveSupport::JSON.decode(v) rescue nil) if k.to_s == 'value' && String===v
